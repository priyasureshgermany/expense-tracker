...
| Reminders | 🔔 in the top bar: what falls due in the next three days, each with the same tick the ledger has — and the same question about which day it was paid. Ticking one off here drops it from the list without opening it. A bill already settled for its period is not asked for again: judged by the month for a rule that falls at most once in one, and by the date it fell on for weekly and every-28-days, which can legitimately come twice. The ↻ re-checks both, for a bill paid somewhere other than here. |
